...
<template>
  <div class="Type">
    <Modal  class="Type"
        ...
        title="批量录入">
        <Row  ref="m1n1">
            <i-col  class="tdtd">类型描述</i-col><i-col  class="tdtd">类型文本</i-col><i-col  class="tdtd">分类</i-col>
        </Row>
        <Row :gutter="10">
          <i-col span="24">
            <Card>
    ...
    <Card>
      <tables
        ...
        size="small"
        search-place="top"
        v-model="stores.Type.data"
        :totalCount="stores.Type.query.totalCount"
        :pageSize="stores.Type.query.pageSize"
        :columns="stores.Type.columns"
                ...
                <Form inline @submit.native.prevent>
                  <FormItem>
                    <Input
                      ...
                      search
                      :clearable="true"
                      v-model="stores.Type.query.kw"
                      placeholder="输入类型描述/搜索..."
                      @on-search="handleSearchType()"
                    >
                      <Select
                        slot="prepend"
                        v-model="stores.Type.query.isDeleted"
                        @on-change="handleSearchType"
                        placeholder="删除状态"
                        style="width:60px;"
                      >
                        <Option
                          v-for="item in stores.Type.sources.isDeletedSources"
                          :value="item.value"
                          :key="item.value"
                        >{{item.text}}</Option>
                      </Select>
                      <Select
                        slot="prepend"
                        v-model="stores.Type.query.status"
                        @on-change="handleSearchType"
                        placeholder="状态"
                        style="width:60px;"
                      >
                        <Option
                          v-for="item in stores.Type.sources.statusSources"
                          :value="item.value"
                          :key="item.value"
                        >{{item.text}}</Option>
                      </Select>
    ...
    <Drawer
      ...
      :styles="styles"
    >
      <Form :model="formModel.fields" ref="formType" :rules="formModel.rules" label-position="top">
        <Row :gutter="16">
                    <Col span="12">
                    ...
                    <Col span="12">
                    <FormItem label="分类" prop="TypeText" >
                      <Input v-model="formModel.fields.TypeText" placeholder="请输入分类"/>
                    </FormItem>
                    </Col>
...
export default {
  ...
  data() {
    return {
      dataorder:["K_Name_kw","Remarks","TypeText"],
      pasteDataArr: [],
      columns: [],
      validated: true,
      errorIndex: 0,
      ...
      stores: {
        Type: {
          ...
          columns: [
            ...
            { title: "分类", key: "typeText",  sortable: "custom" },    
            {
              title: "状态",
              key: "status",
              align: "center",
  ...
  methods: {
    ...
    loadTypeList() {
      getTypeList(this.stores.Type.query).then(res => {
        this.stores.Type.data = getDateMore(res.data.data,1,[]);
        this.stores.Type.query.totalCount = res.data.totalCount;
      });
    },
    ...
    handleEdit(params) {
      ...
      this.handleResetFormType();
      this.doLoadAllForinkey();
      this.doLoadType(params.row.id);
    },
    ...
    handleSubmitType() {
      let valid = this.validateTypeForm();
      let o=this;
    ...
    doCreateType() {
      ...
      createType(this.formModel.fields).then(res => {
    ...
    doEditType() {
      editType(this.formModel.fields).then(res => {
        if (res.data.code === 200) {
          this.$Message.success(res.data.message);
          this.loadTypeList();
        } else {
    ...
    validateTypeForm() {
      ...
      return new Promise(function(resolve, reject){
        o.$refs["formType"].validate(valid => {
          if (!valid) {
    ...
    handlePageChanged(page) {
      this.stores.Type.query.currentPage = page;
      this.loadTypeList();
    },
    handlePageSizeChanged(pageSize) {
      this.stores.Type.query.pageSize = pageSize;
      this.loadTypeList();
    },
    handleInputData(){
      this.modal1=true;

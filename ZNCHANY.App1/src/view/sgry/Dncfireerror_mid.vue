...
<template>
  <div class="Fireerror_mid">
    <Modal  class="Fireerror_mid"
        v-model="modal1"
        width=800
        title="批量录入">
        <Row  ref="m1n1">
            <i-col  class="tdtd"></i-col><i-col  class="tdtd">实际时间</i-col><i-col  class="tdtd">异常小类ID</i-col><i-col  class="tdtd">状态(0:正常 1:异常)</i-col><i-col  class="tdtd">备注</i-col><i-col  class="tdtd"></i-col>
        </Row>
        <Row :gutter="10">
          <i-col span="24">
            <Card>
    ...
    <Card>
      <tables
        ...
        size="small"
        search-place="top"
        v-model="stores.Fireerror_mid.data"
        :totalCount="stores.Fireerror_mid.query.totalCount"
        :pageSize="stores.Fireerror_mid.query.pageSize"
        :columns="stores.Fireerror_mid.columns"
                ...
                <Form inline @submit.native.prevent>
                  <FormItem>
                    <Input
                      ...
                      search
                      :clearable="true"
                      v-model="stores.Fireerror_mid.query.kw"
                      placeholder="输入搜索..."
                      @on-search="handleSearchFireerror_mid()"
                    >
                      <Select
                        slot="prepend"
                        v-model="stores.Fireerror_mid.query.isDeleted"
                        @on-change="handleSearchFireerror_mid"
                        placeholder="删除状态"
                        style="width:60px;"
                      >
                        <Option
                          v-for="item in stores.Fireerror_mid.sources.isDeletedSources"
                          :value="item.value"
                          :key="item.value"
                        >{{item.text}}</Option>
                      </Select>
                      <Select
                        slot="prepend"
                        v-model="stores.Fireerror_mid.query.status"
                        @on-change="handleSearchFireerror_mid"
                        placeholder="状态"
                        style="width:60px;"
                      >
                        <Option
                          v-for="item in stores.Fireerror_mid.sources.statusSources"
                          :value="item.value"
                          :key="item.value"
                        >{{item.text}}</Option>
                      </Select>
    ...
    <Drawer
      ...
      :styles="styles"
    >
      <Form :model="formModel.fields" ref="formFireerror_mid" :rules="formModel.rules" label-position="top">
        <Row :gutter="16">
                    <Col span="12">
                    ...
                    <Col span="12">
                    <FormItem label="异常小类ID" prop="Etype">
                      <i-switch size="large" v-model="formModel.fields.Etype" :true-value="1" :false-value="0">
                    ...
                    <Col span="12">
                    <FormItem label="状态(0:正常 1:异常)" prop="Evalue">
      ...
      <div class="demo-drawer-footer">
        <Button icon="md-checkmark-circle" type="primary" @click="handleSubmitFireerror_mid">保 存</Button>
...
import {
  getFireerror_midList,
  createFireerror_mid,
  loadFireerror_mid,
  editFireerror_mid,
  deleteFireerror_mid,
  batchCommand,
  batchCreateFireerror_mid
} from "@/api/ZNRS/Dncfireerror_mid";
export default {
  name: "ZNRS_Fireerror_mid_page",
  components: {
    Tables,PasteEditor
  },
  data() {
    return {
      dataorder:["DncType_Name","RealTime","Etype","Evalue","Remarks","DncBoiler_Name"],
      pasteDataArr: [],
      columns: [],
      validated: true,
      errorIndex: 0,
      ...
      stores: {
        Fireerror_mid: {
          ...
          columns: [
            ...
            { title: "异常小类ID", key: "etype",  sortable: "custom" },    
            { title: "状态(0:正常 1:异常)", key: "evalue",  sortable: "custom" },    
  ...
  methods: {
    ...
    loadFireerror_midList() {
      getFireerror_midList(this.stores.Fireerror_mid.query).then(res => {
        this.stores.Fireerror_mid.data = getDateMore(res.data.data,1,["realTime",]);
        this.stores.Fireerror_mid.query.totalCount = res.data.totalCount;
      });
    },
    ...
    handleEdit(params) {
      ...
      this.handleResetFormFireerror_mid();
      this.doLoadAllForinkey();
      this.doLoadFireerror_mid(params.row.id);
    },
    ...
    handleSubmitFireerror_mid() {
      let valid = this.validateFireerror_midForm();
      let o=this;
      valid.then(function(data){ 
        if (data) {
          if (o.formModel.mode === "create") {
            o.doCreateFireerror_mid();
          }
          if (o.formModel.mode === "edit") {
            o.doEditFireerror_mid();
          }
        }
      });
    },
    handleResetFormFireerror_mid() {
      this.$refs["formFireerror_mid"].resetFields();
    },
    doCreateFireerror_mid() {
      
      createFireerror_mid(this.formModel.fields).then(res => {
    ...
    doEditFireerror_mid() {
      editFireerror_mid(this.formModel.fields).then(res => {
        if (res.data.code === 200) {
          this.$Message.success(res.data.message);
          this.loadFireerror_midList();
        } else {
    ...
    validateFireerror_midForm() {
      let _valid = false;
      let o=this;
      return new Promise(function(resolve, reject){
        o.$refs["formFireerror_mid"].validate(valid => {
          if (!valid) {
    ...
    doLoadFireerror_mid(id) {
      loadFireerror_mid({ code: id+"" }).then(res => {
   ...
   //     op.DncBoiler_Name=parseInt(op.DncBoiler_Name);
    ...
    doDelete(ids) {
      ...
      deleteFireerror_mid(ids).then(res => {
        if (res.data.code === 200) {
          this.$Message.success(res.data.message);
          this.loadFireerror_midList();
        } else {
    ...
    handlePageChanged(page) {
      this.stores.Fireerror_mid.query.currentPage = page;
      this.loadFireerror_midList();
    },
    handlePageSizeChanged(pageSize) {
      this.stores.Fireerror_mid.query.pageSize = pageSize;
      this.loadFireerror_midList();
    },
    handleInputData(){
      this.modal1=true;
    ...
    handleSortChange(column){
      this.stores.Fireerror_mid.query.sort= [
        {
          direct: column.order,
          field: column.key
        }

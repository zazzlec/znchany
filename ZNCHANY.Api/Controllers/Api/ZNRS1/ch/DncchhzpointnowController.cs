...
using ZNCHANY.Api.RequestPayload.Rbac.Chhzpointnow;
using ZNCHANY.Api.ViewModels.Rbac.Dncchhzpointnow;
...
namespace ZNCHANY.Api.Controllers.Api.ZNCHANY1
{
    ...
    [ApiController]
    //[CustomAuthorize]
    public class DncchhzpointnowController : ControllerBase
    {
        ...
        public DncchhzpointnowController(ZNCHANYDbContext dbContext, IMapper mapper)
        {
        ...
        public IActionResult List()
        {
            using (_dbContext)
            {
                var list = _dbContext.Dncchhzpointnow.ToList();
        ...
        /// <returns></returns>
        [HttpPost]
        public IActionResult List(DncchhzpointnowRequestPayload payload)
        {
            var response = ResponseModelFactory.CreateResultInstance;
            using (_dbContext)
            {
                var query = _dbContext.Dncchhzpointnow.AsQueryable();
                //模糊查询
                ...
                var data = list.Select(_mapper.Map< Dncchhzpointnow, DncchhzpointnowJsonModel>);
        ...
        public IActionResult Create(DncchhzpointnowCreateViewModel model)
        {
            var response = ResponseModelFactory.CreateInstance;
            using (_dbContext)
            {
                var entity = _mapper.Map< DncchhzpointnowCreateViewModel, Dncchhzpointnow>(model);
                if (string.IsNullOrEmpty(model.DncType_Name))
                {
                    entity.DncType = _dbContext.Dnctype.FirstOrDefault(x => (x.K_Name_kw + "") == "无");
                ...
                _dbContext.Dncchhzpointnow.Add(entity);
        ...
        public IActionResult Edit(string code)
        {
            using (_dbContext)
            {
                var entity = _dbContext.Dncchhzpointnow.FirstOrDefault(x => x.Id ==  int.Parse(code));
                var response = ResponseModelFactory.CreateInstance;
                response.SetData(_mapper.Map< Dncchhzpointnow, DncchhzpointnowCreateViewModel>(entity));
                return Ok(response);
            }
        }
        ...
        [HttpPost]
        [ProducesResponseType(200)]
        public IActionResult Edit(DncchhzpointnowEditViewModel model)
        {
            var response = ResponseModelFactory.CreateInstance;
            using (_dbContext)
            {
                ...
                var entity = _dbContext.Dncchhzpointnow.FirstOrDefault(x => x.Id == model.Id);
        ...
        private ResponseModel UpdateIsDelete(CommonEnum.IsDeleted isDeleted, string ids)
        {
            using (_dbContext)
            {
                if (ToolService.DbType.Equals("mysql")){
                    ...
                    var sql1= string.Format("delete from  Dncchhzpointnow  WHERE id IN ({0}) and IsDeleted=1", parameterNames);
                    _dbContext.Database.ExecuteSqlCommand(sql1);
                    
                    var sql = string.Format("UPDATE Dncchhzpointnow SET IsDeleted=@IsDeleted WHERE id IN ({0})", parameterNames);
        ...
        private ResponseModel UpdateStatus(UserStatus status, string ids)
        {
            using (_dbContext)
            {
                if (ToolService.DbType.Equals("mysql")){
                    var parameters = ids.Split(",").Select((id, index) => new MySqlParameter(string.Format("@p{0}", index), id)).ToList();
                    var parameterNames = string.Join(", ", parameters.Select(p => p.ParameterName));
                    var sql = string.Format("UPDATE Dncchhzpointnow SET Status=@Status WHERE id IN ({0})", parameterNames);
        ...
        public IActionResult BatchCreate(string fsts)
        {
            ...
            try
            {
                using (TransactionScope scope = new TransactionScope())
                {
                    using (_dbContext)
                    {
                        KeyValuePair<string, List< DncchhzpointnowCreateViewModel>> res = ValidateJson.Validation< DncchhzpointnowCreateViewModel>(fsts);

                        if (res.Key.Equals("ok"))
                        {
                            List< DncchhzpointnowCreateViewModel> arr = res.Value;
                            foreach ( DncchhzpointnowCreateViewModel item in arr)
                            {
                                ...
                                var entity = _mapper.Map< DncchhzpointnowCreateViewModel, Dncchhzpointnow>(item);
